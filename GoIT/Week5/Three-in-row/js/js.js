	// var color=['images/mage.svg','images/rogue.svg','images/hunter.svg','images/shaman.svg','images/warrior.svg','images/druid.svg'];
	var color=['red','blue','green','yellow','orange','violet'];
	
	var side = +prompt("Three in row game! Enter the size of board please:", 7);
	var restartFlag = 0;
	var checkbox = true;
	var score = 0;
			...
			setTimeout(function () {
				...
				function JewClick() {
					if (click === 0) {
						...
						var jewSecond = event.target;
						var data = parseInt(jewFirst.getAttribute("data-n"));
						var data2 = parseInt(jewSecond.getAttribute("data-n"));
						if (data+side === data2 || data-side === data2 || data+1 === data2 || data-1 === data2) {
							if (data+side === data2) {
								jewSecond.classList.add("anim-class-down");
								jewFirst.classList.add("anim-class-up");
							}
							else if (data-side === data2) {
								jewSecond.classList.add("anim-class-up");
								jewFirst.classList.add("anim-class-down");
							}
							else if (data+1 === data2) {
								jewSecond.classList.add("anim-class-left");
								jewFirst.classList.add("anim-class-right");
							}
							else if (data-1 === data2) {
								jewSecond.classList.add("anim-class-right");
								jewFirst.classList.add("anim-class-left");
							}
							jewFirst.style.border = 'none';
							setTimeout(function ()
							{
								jewSecond.classList.remove("anim-class-right","anim-class-left","anim-class-down","anim-class-up");
								jewFirst.classList.remove("anim-class-right","anim-class-left","anim-class-down","anim-class-up");
	
								var firstColor = jewFirst.getAttribute('type');
								jewFirst.setAttribute('type', jewSecond.getAttribute('type'));
								jewFirst.style.backgroundColor = color[jewSecond.getAttribute('type')];
								
								var secondColor = jewSecond.getAttribute('type');
								jewSecond.setAttribute('type', firstColor);
								jewSecond.style.backgroundColor = color[firstColor];
								if (!CheckThree()) {
									jewFirst.setAttribute('type', firstColor);
									jewFirst.style.backgroundColor = color[firstColor];
									jewFirst.style.border = 'none';
									jewSecond.setAttribute('type', secondColor);
									jewSecond.style.backgroundColor = color[secondColor];
								}
								click = 0;
								jewFirst = 0;
							}, 1000);
	...
	function DestroyThreeRow(count, array, row) {
		...
		for (var i = 0; i < count; i++) {
			for (var j = row-1; j > 0; j--) {
				...
				// document.getElementsByClassName("row"+(j+1))[index_st+i].firstChild.style.backgroundImage='url("' + color[type_temp] + '")';
			}
			...
			// document.getElementsByClassName("row1")[index_st + i].firstChild.style.backgroundImage='url("' + color[newTypeRand] + '")';
		}
		setTimeout(function() {CheckThree()},800);
	}
		...
		var index_st = array[1]+2;
				for (var j = index_st; j > 0; j--) {
					if (j > index_st - (index_st - count)) {
						...
						// document.getElementsByClassName("row" + (j))[col - 1].firstChild.style.backgroundImage='url("' + color[type_temp] + '")';
					}
					else {
						var newTypeRand = Math.floor(Math.random()*6);
						...
						// document.getElementsByClassName("row"+j)[col-1].firstChild.style.backgroundImage='url("' + color[newTypeRand] + '")';
					}
				}
